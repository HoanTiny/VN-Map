...
function timeAgo(iso: string): string {
  ...
  if (diff < 604800) return RELATIVE_TIME.format(-Math.floor(diff / 86400), "day");
  return RELATIVE_TIME.format(-Math.floor(diff / 604800), "week");
}
    ...
    <div className="rounded-3xl bg-surface-1 ring-1 ring-border p-5 md:p-6 shadow-sm">
      <div className="mb-5 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-overline text-success">Đang diễn ra</span>
        </div>
        <span className="text-caption text-text-subtle">{items.length} hoạt động</span>
      </div>

      {empty ? (
        <p className="py-8 text-center text-body-sm text-text-muted">
          Chưa có hoạt động nào. Hãy đóng góp địa điểm hoặc viết đánh giá đầu tiên!
        </p>
      ) : (
...
function ActivityCard({ item }: { item: ActivityItem }) {
  const cat = useMemo(() => {
    ...
    return c ? { ...c, label: c.labelVi || c.label } : null;
  }, [item.category]);
    ...
    >
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-surface-2">
        ...
        <div className="absolute left-2.5 top-2.5">
          {item.kind === "place" ? (
            <Badge variant="brand" className="text-[10px] shadow-sm">Mới được duyệt</Badge>
          ) : (
      ...
      <div className="flex-1 px-3.5 py-3">
        {item.kind === "place" ? (
          <p className="text-body-sm text-text-muted">
            {cat?.label ?? "Địa điểm mới"}
            {item.province ? ` · ${item.province}` : ""}
          </p>
        ) : (
          <p className="line-clamp-2 text-body-sm text-text-muted">
            <span className="font-medium text-text">{item.author ?? "Khách"}</span>
            {item.body ? `: "${item.body}"` : ""}
          </p>
        )}
      </div>
