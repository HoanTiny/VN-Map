  ...
  return (
    <article className="pb-24">
      ...
      <section className="relative isolate overflow-hidden pt-24 md:pt-28">
        ...
        <div className="container pb-16 text-white">
          <Reveal>
            ...
            >
              <ArrowLeft size={14} /> Vùng miền
            </Link>
          </Reveal>
          <Reveal delay={0.05}>
            <p className="mt-6 text-overline text-white/80">VÙNG</p>
            ...
            <div className="mt-4 flex items-center gap-2">
              <Badge variant="neutral" className="bg-white/15 text-white">
                {provs.length} tỉnh thành
              </Badge>
              <Badge variant="neutral" className="bg-white/15 text-white">
                {provs.reduce((sum, p) => sum + (placeCounts.get(p.name) ?? 0), 0)} địa điểm
              </Badge>
            </div>
          </Reveal>
        </div>
      ...
      <section className="container py-12">
        <Reveal>
          <h2 className="mb-6 font-display text-h2 text-text">Các tỉnh thành</h2>
        </Reveal>
          ...
          {provs.map((p) => (
            <StaggerItem key={p.slug}>
              <ProvinceCard
                ...
                tagline={p.tagline}
                cover={p.cover}
                count={placeCounts.get(p.name) ?? 0}
              />
            </StaggerItem>
          ))}
        </Stagger>
        ...
        <Reveal>
          <div className="mt-12 rounded-2xl border border-border bg-surface p-6 text-center">
            <p className="font-display text-h3 text-text">Khám phá toàn vùng trên bản đồ</p>
            <p className="mt-1 text-body-sm text-text-muted">
              Xem tất cả địa điểm thuộc {r.label} trên bản đồ tương tác.
            </p>
